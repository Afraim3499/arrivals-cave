...
async function main() {
    console.log('Seeding Blog 6...');

    const wordCount = contentMarkdown.split(/\s+/).length;
    console.log(`Word count for Blog 6: ${wordCount} words`);
    ...
    if (error) {
        console.error('Error updating blog 6:', error);
        process.exit(1);
    } else {
        console.log('✅ Successfully updated Blog 6!');
    }
    process.exit(0);
}
